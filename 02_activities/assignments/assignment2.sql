...
SELECT market_date, total_sales, 'BEST' AS type
FROM ranked
WHERE best_rank = 1
UNION
SELECT market_date, total_sales, 'WORST' AS type
FROM ranked
WHERE worst_rank = 1;
...
SELECT
    v.vendor_name,
    p.product_name,
    SUM(5 * p.price) AS revenue_per_product
FROM (
    SELECT DISTINCT vendor_id, product_id, quantity
...
VALUES (
    999, 'Apple Pie', 'L', 'unit', CURRENT_TIMESTAMP
);
